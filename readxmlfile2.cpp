...
{
    vector<std::pair<Point, Point>> list = listPoints(e.shapeofedge.Point1,e.shapeofedge.Point2, x);
    cout<<list.size()<<endl;
    int i = 0;
    part a;
    for (auto& point : list)
    {
        ...
        a.name = e.id;
        a.i=i;
        a.Point1 = make_pair(point.first.first,point.first.second);
        a.Point2 = make_pair(point.second.first,point.second.second);
        allpart.push_back(a);
        i++;
    }
}
...
{
    vector<Point> points = j.points;
    cout<<points.size()<<endl;
    int i;
    for ( i = 0; i < points.size()-1; i++)
    {
        vector<std::pair<Point, Point>> list = listPoints(points.at(i), points.at(i+1), x);
        int n = 0;
        for (auto& point : list)
        {
            //cout<< j.id << " " << n << "_" << point.first.first << "," << point.first.second << "_" << point.second.first << "," << point.second.second << "\n";
            part a;
            a.name = j.id;
            a.i = n;
